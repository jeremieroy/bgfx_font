...
class RectanglePackerCube
{
public:
	RectanglePackerCube(){}
	RectanglePackerCube(uint32_t sideWidth) { init(sideWidth); }
	/// non constructor initialization
	void init(uint32_t sideWidth)
	{
		for(int i=0; i<6;++i)
		{
			m_sides[i].init(sideWidth,sideWidth);
		}
	}
	/// find a suitable position for the given rectangle 
	/// @return true if the rectangle can be added, false otherwise	
	bool addRectangle(uint16_t width, uint16_t height, uint16_t& outX, uint16_t& outY, uint16_t& outFaceIndex )
	{
		for(outFaceIndex=0; outFaceIndex<6;++outFaceIndex)
		{
			if( m_sides[outFaceIndex].addRectangle(width,height,outX,outY)) return true;
		}
		return false;
	}
	/// return the used surface in squared unit
	...
	{ 
		uint32_t totalSurface = 0;
		for(int i=0; i<6;++i)
		{
			totalSurface+= m_sides[i].getUsedSurface();
		}
		return totalSurface;	
	}
	/// return the total available surface in squared unit
	uint32_t getTotalSurface() { return m_sides[0].getTotalSurface() * 6; }
	/// return the usage ratio of the available surface [0:1]
	float getUsageRatio()
	{
		float totalRatio = 0;
		for(int i=0; i<6;++i)
		{
			totalRatio+= m_sides[i].getUsageRatio();
		}
